...
% Figure; Binomial model fitting
figure, hold on; 
b = bar(TRUE_WASHU','FaceColor','flat');
xticks([1:length(VAF)]);
...
for i = 1:length(n_total)
    ...
    plot(x+(i-2.5)*0.18,RR_WASHU_BB(i,:),'rs:','MarkerSize',7);
    plot(x+(i-2.5)*0.18,RR_WASHU_BI_ER(i,:),'bs:','MarkerSize',7);
    plot(x+(i-2.5)*0.18,RR_WASHU_BB_ER(i,:),'ys:','MarkerSize',7);
    plot(x+(i-2.5)*0.18,RR_WASHU_BI_ER_PO(i,:),'gs:','MarkerSize',7);
    plot(x+(i-2.5)*0.18,RR_WASHU_BB_ER_PO(i,:),'ms:','MarkerSize',7);
end
% legend({'WASHU','BCM','NYGC','UW','SIM485','SIM4000',...
%     'Binomial w/SQ-ER','','','','','',...
%     'Binomial','','',''},'Location','southwest')
legend({'WASHU',...
    'Binomial','Beta-binomial','Binomial w/SQ-ER','BB w/ SQER',...
    'Binom w/ POI SQER','BB w/ POI SQER'},'Location','southwest')
ylim([0 1]);
...
figure,
h = heatmap(RR');
title(['Recall rate (%; N by VAF) with k0 = ' num2str(k0)])
h.YLabel = 'VAF(%)';
